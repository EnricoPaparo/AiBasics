...
Il punto di partenza di ogni sistema ben progettato è una definizione chiara del problema, prima di qualsiasi scelta tecnica — esattamente la disciplina costruita nella Lezione 8.1.
...
- Il materiale è spesso ambiguo o contraddittorio
  (rilevato, non risolto autonomamente — Lezione 7.6)
- Serve sempre una revisione umana prima che il documento
  raggiunga il team di architettura (Lezione 8.4)
...
Seguendo la struttura della Lezione 7.2, il progetto richiede almeno tre agent package completi:

```
/agente-requirement-analyst/
├── agent.yaml
├── agent_card.yaml        (Lezione 7.3 — dichiara "NON_fa
│                            decisioni di business strategiche")
├── prompts/
│   └── system.md           (Lezione 7.4)
├── skills/
│   └── rilevamento-ambiguita.md  (Lezione 7.7)
├── schemas/
│   ├── input_schema.json   (Lezione 7.5)
│   └── output_schema.json
└── evals/
    └── casi_test.yaml       (Lezione 8.5)

/agente-critic-requisiti/
├── agent.yaml
├── agent_card.yaml
├── prompts/
│   └── system.md            (rubrica di valutazione, Lezione 8.3)
└── schemas/
    └── output_schema.json   (ValutazioneReview, Lezione 8.3)
...
# 2. Costruzione del grafo (Lezione 8.2)
...
# Nodi paralleli di estrazione (Lezione 8.2, Sezione 3)
...
grafo.add_conditional_edges(
    "review", decidi_dopo_review,  # Lezione 8.3
    {
...
# 3. Compilazione con checkpointing per HITL asincrono (Lezione 8.4)
...
Per completare il sistema secondo l'ambizione dichiarata fin dall'inizio del corso, aggiungiamo gli ultimi due componenti del Capitolo 9:

```python
# Self-critique opzionale PRIMA del review esterno (Lezione 9.1),
# come primo filtro economico
def nodo_requirement_analyst(stato: StatoProgetto) -> dict:
    output = genera_requisiti(stato)
    output_validato = applica_self_reflection(output)  # Lezione 9.1
...
# riassorbimento della conoscenza (Lezione 9.3)
...
Prima di considerare il progetto concluso, applichiamo la disciplina della Lezione 8.5: una test suite che copre almeno i casi fondamentali.
...
- id: "caso-003-audio-corrotto"
  atteso: {gestione_errore: "fallback_attivato"}  # Lezione 6.5
```
...
- L'estrazione da audio e immagini (nodi paralleli) dipende
  ...
  parzialmente da fallback, Lezione 6.5)

- Il Critic-Agent condivide il modello sottostante con il
  Requirement Analyst — il rischio di echo chamber
  (Lezione 9.1) non è eliminato, solo ridotto rispetto
  alla pura self-reflection

- Il riassorbimento della conoscenza (Lezione 9.3) richiede
...
1. I nodi di estrazione (PDF/audio/immagini) dovranno
   ...
   tuo ambiente opencode (Lezione 5.4, Function Calling,
   o Lezione 5.5, server MCP se disponibili)

2. Il meccanismo di notifica per il checkpoint umano
   (Lezione 8.4) dovrà integrarsi con il tuo flusso di
   lavoro reale — email, dashboard, o altro canale

3. La skill "rilevamento-ambiguita" (Lezione 7.7) dovrà
...
- **(a) nodi paralleli** → LangGraph, Lezione 8.2.
- **(b) review layer (Critic-Agent)** → Lezione 8.3.
- **(c) checkpoint umano (HITL)** → Lezione 8.4.
- **(d) handoff con contratto** → Lezioni 7.6 (handoff) + 6.5 (contratti).
- **(e) registrazione episodi (memoria episodica)** → Lezione 9.3.
...
Se fossero due definizioni separate, nulla garantirebbe che restino allineate: l'Analyst potrebbe produrre un campo `valore_vendite` mentre l'Architect ne attende `importo` — riproducendo esattamente il problema di **coerenza terminologica** della Lezione 6.4. È un errore che si scopre tardi, a valle, ed è difficile da diagnosticare.

Condividendo **un unico contratto** (un solo schema, Lezione 7.5), l'output di A e l'input di B sono **per costruzione** lo stesso formato, **verificabile automaticamente**. Una modifica al contratto si propaga a entrambi. È il principio che l'intero Capitolo 7 è stato costruito per garantire: violarlo, anche in un progetto di portfolio, riprodurrebbe l'errore che il corso insegna a prevenire.
...
- **Echo chamber del Critic-Agent:** se condivide il modello sottostante col Requirement Analyst, il rischio di bias condiviso è ridotto ma non eliminato (Lezione 9.1).
- **Dipendenza dalla qualità delle decisioni umane storiche:** il riassorbimento (9.3) migliora il sistema solo se le decisioni umane registrate erano buone — altrimenti propaga gli errori.
