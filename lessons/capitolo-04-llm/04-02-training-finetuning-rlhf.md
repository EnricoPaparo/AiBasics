...
FINE-TUNING (supervisionato — riusa il concetto
             dell'apprendimento supervisionato, Lezione 3.2)
...
4. Il modello principale viene poi ulteriormente
   ...
   (questo è, letteralmente, l'apprendimento per
    rinforzo descritto nella Lezione 3.2: agente = il
...
Questo terzo livello — usare un modello già pronto, eccellente, addestrato da altri, e costruire attorno a esso logica, strumenti, memoria, orchestrazione — è esattamente l'approccio che adotteremo a partire dal Capitolo 5. Non dovremo mai preoccuparci di addestrare nulla da zero: il nostro lavoro sarà costruire sistemi intelligenti **attorno** a modelli già eccellenti, sfruttandone le capacità tramite le tecniche che vedremo nei capitoli successivi.
...
3. **Ammette di non sapere qualcosa, invece di inventare una risposta plausibile ma falsa** — quando questo accade (non sempre, come vedremo nella Lezione 4.5), è spesso il risultato di RLHF che ha premiato l'onestà sui propri limiti rispetto a risposte sicure di sé ma infondate
...
3. Rileggi la definizione di apprendimento per rinforzo data nella Lezione 3.2 (agente, ambiente, ricompensa). Identifica esplicitamente questi tre elementi nel processo RLHF descritto in questa lezione.
...
Nella Lezione 3.2 l'apprendimento per rinforzo era: agente, ambiente, ricompensa. Identifica questi tre elementi nel processo RLHF. Poi spiega perché spesso è più facile *confrontare* due risposte che *scrivere* la risposta ideale.
...
**Viene da:** Lezione 4.1 — qui vediamo come il "semplice" compito di prevedere il token successivo, dopo il pre-training, viene ulteriormente plasmato in un comportamento utile e sicuro.

**Porta a:** Lezione 4.3 (Anatomia di un prodotto AI) — vedremo come un modello istruito come quello descritto qui viene integrato in un prodotto completo come Claude.ai.

**Ritroverai questi concetti in:** Lezione 3.2 (Machine Learning) — RLHF è un'applicazione diretta e concreta dell'apprendimento per rinforzo descritto lì in astratto. Lezione 9.1 (Self-Reflection) — il principio di usare un modello "giudice" per valutare risposte, centrale nel reward model di RLHF, ritornerà in una forma adattata quando un agente valuterà i propri stessi output.
